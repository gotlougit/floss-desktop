...
microphone PCM retaining a balanced, non-silent 500 Hz tone after rate conversion,
stable PipeWire node IDs, a mono source channel, source volume retained across the
profile round trip, and recording restart debounce.
...
capture and validate fresh microphone PCM after reconnect, remove/restore HFP
capability, and replace the daemon bus owner while the same
...
`--cases vlc --vlc /nix/store/.../bin/vlc` additionally checks an initially
corked PulseAudio client: VLC must complete a three-second local WAV after an
idle interval without another stream waking the graph. The mock must receive
nonzero PCM. Use an already installed VLC; the test downloads nothing.
...
The CVSD and mSBC cases validate the respective 8 and 16 kHz PCM contracts on
the host side of Floss; they do not encode/decode SCO bitstreams or negotiate
with a controller and headset.
